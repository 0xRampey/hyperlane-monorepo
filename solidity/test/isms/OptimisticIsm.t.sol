...
contract OptimisticIsmTest is Test {
    ...
    function addWatchersAndPreVerifyIsm(
        ...
        ism = IOptimisticIsm(
            factory.deploy(addresses, m, fraudWindow, address(subIsm))
        );
        return addresses;
    }
    ...
    function testVerifyFailWithLessThanMOfNWatchers(
        uint32 destination,
        bytes32 recipient,
        bytes calldata body,
        uint8 m,
    ...
    function testVerifyFailBeforeFraudWindow(
        uint32 destination,
        bytes32 recipient,
        bytes calldata body,
        uint8 m,
    ...
    function testVerifyAfterFraudWindow(
        uint32 destination,
        bytes32 recipient,
        bytes calldata body,
        uint8 m,
        ...
        // Fast forward past the fraud window!
        vm.roll(block.number + fraudWindow);
        ism.verify(metadata, message);
    }
}
